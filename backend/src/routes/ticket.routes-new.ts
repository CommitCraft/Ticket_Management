...
ticketRouter.post('/', protect, permitPermissions(PERMISSIONS.TICKET_WRITE), (req, res, next) => {
...
}, validateBody(createTicketSchema), createTicket);
...
ticketRouter.post('/:id/replies', protect, permitPermissions(PERMISSIONS.TICKET_WRITE), (req, res, next) => {
  fileUpload(req, res, (err) => {
    if (err) {
      return handleUploadError(err, req, res, next);
    }
...
}, validateBody(replyTicketSchema), replyToTicket);
